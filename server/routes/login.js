...
router.post('/register', async (req, res) => {
  ...
  const sameUsernameExists = await User.findOne({ username })
    .catch((err) => res.status(500).json({ message: err.toString() }))
  if (sameUsernameExists) return res.status(400)
...
router.get('/locked', passport.authenticate('jwt', { session: false }),
  (req, res) => {
    res.status(200)
      .json(
        { message: `Hey ${req.user.username}, you found the secret page!` })
  })

router.get('/authenticated', passport.authenticate('jwt', { session: false }),
  (req, res) => {
